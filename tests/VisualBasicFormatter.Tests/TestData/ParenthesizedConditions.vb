Module ParenthesizedConditions
    ...
    Public Function Classify(ByVal candidate As Contract) As String
        If (candidate.IsActive AndAlso candidate.HasValidSignature AndAlso candidate.RemainingTermInMonths > 0) Then
            Return "A"
        ElseIf (candidate.IsPending AndAlso candidate.HasValidSignature AndAlso candidate.RemainingTermInMonths >= 0) Then
            Return "B"
        Else
            Return "C"
        End If
    ...
    Public Function NegatedCall(ByVal candidate As Contract) As Boolean
        If (Not Evaluate(candidate.PrimaryHolderName, candidate.SecondaryHolderName, candidate.BranchName, candidate.ProductName)) Then
            Return True
        End If
    ...
    Public Function OperatorEndingInCall(ByVal candidate As Contract) As Boolean
        If (candidate.IsActive AndAlso Evaluate(candidate.PrimaryHolderName, candidate.SecondaryHolderName, candidate.BranchName)) Then
            Return True
        End If
